	...
	public class MapGenerator {
		...
		public char[, ] Display() {
			...
			*/
			...
			char[, ] res = new char[GRID_WIDTH, GRID_HEIGHT];
			...
			for (int x = 0; x < GRID_WIDTH; ++x)
				for (int y = 0; y < GRID_HEIGHT; ++y) {
					res[x, y] = tileset[(int) grid[x, y].tile];
				}
			return res;
		}
		...
		public int NUM_OF_TOWERS = 0;
		public int NUM_OF_DECORS = 0;
		const int NUM_OF_DIRECTIONS = 4;
		private float _decorPercent = 1.0f;
		private float _towerPercent = 1.0f;
		...
		private Pair _positionSplit;

		// array that contains map
		private Tile[, ] grid = new Tile[GRID_WIDTH, GRID_HEIGHT];
		public Tile[, ] Grid{ get => grid;}
		private TileType[, ] tileMap = new TileType[GRID_WIDTH, GRID_HEIGHT];

		// bool to check for adjacency
		private bool _isTurnTile = false;

		// int array for counting distance till path tile
		private int[] _emptyCount = new int[NUM_OF_DIRECTIONS];

		#endregion

		#region Properties

		public float DecorPercent {
			get { return _decorPercent; }
			set { _decorPercent = value; }
		}

		public float TowerPercent {
		...
		/// </summary>
		/// <returns></returns>
		public TileType[, ] GetMapAsTileType() {
			for (int x = 0; x < GRID_WIDTH; x++) {
				for (int y = 0; y < GRID_HEIGHT; y++) {
		...
		//public  TileType[,] GenerateMap(MapLength length, bool splitPath, float towerPercent, float decorPercent)
		...
		public TileType[, ] GenerateMap(MapSize size, MapLength length, bool splitPath, float towerPercent,
			float decorPercent) {
			StartUp();
			TowerPercent = towerPercent;
			DecorPercent = decorPercent;
		...
		private bool MakePath(MapSize size, MapLength length) {
			bool map = false;
				...
				case MapSize.Small:
					...
					switch (length) {
						...
						case MapLength.Long:
							...
							map = RandomMapGenerator(50, 50, 20);
							break;
						default:
							break;
					}
		...
		/// <returns></returns>
		private bool RandomMapGenerator(int min, int max, int tower_freq) {
		...
		/// </summary>
		/// <returns></returns>
		private bool CheckTileAdjacency() {
		...
		/// <param name="y"></param> 
		/// <returns></returns>
		private bool HasAdjacentTile(int x, int y) {
			// checks for non-connective tiles to the top
			if (x > 0)
				if (grid[x - 1, y].tile == TileType.HorizPath || grid[x - 1, y].tile == TileType.ElbowLeftUp ||
					grid[x - 1, y].tile == TileType.ElbowRightUp ||
					grid[x - 1, y].tile == TileType.SplitULR)
					return true;
			// checks for non-connective tiles to the bottom
			if (x < GRID_HEIGHT - 1)
				if (grid[x + 1, y].tile == TileType.HorizPath || grid[x + 1, y].tile == TileType.ElbowLeftDown ||
					grid[x + 1, y].tile == TileType.ElbowRightDown ||
					grid[x + 1, y].tile == TileType.SplitDLR)
					return true;
			// checks for non-connective tiles to the left
			if (y > 0)
				if (grid[x, y - 1].tile == TileType.VertPath || grid[x, y - 1].tile == TileType.ElbowLeftDown ||
					grid[x, y - 1].tile == TileType.ElbowLeftUp ||
					grid[x, y - 1].tile == TileType.SplitUDL)
					return true;
			// checks for non-connective tiles to the right
			if (y < GRID_WIDTH - 1)
				if (grid[x, y + 1].tile == TileType.VertPath || grid[x, y + 1].tile == TileType.ElbowRightDown ||
					grid[x, y + 1].tile == TileType.ElbowRightUp ||
				...
				while (_positionSplit.y != End.y || _positionSplit.x != End.x) {
					...
					switch (rNum) {
						...
						case (int) Direction.Up:
							...
							if (_positionSplit.y > 0 && Math.Abs(_positionSplit.y - 1 - End.y) <
								Math.Abs(_positionSplit.y - End.y))
								_positionSplit.y -= 1;
							break;
							// move down
						case (int) Direction.Down:
							if (_positionSplit.y < GRID_HEIGHT - 1 && Math.Abs(_positionSplit.y + 1 - End.y) <
								Math.Abs(_positionSplit.y - End.y))
								_positionSplit.y += 1;
							break;
							// move left
						case (int) Direction.Left:
							if (_positionSplit.x > 0 && Math.Abs(_positionSplit.x - 1 - End.x) <
								Math.Abs(_positionSplit.x - End.x))
								_positionSplit.x -= 1;
							break;
							// move right
						case (int) Direction.Right:
							if (_positionSplit.x < GRID_WIDTH - 1 && Math.Abs(_positionSplit.x + 1 - End.x) <
								Math.Abs(_positionSplit.x - End.x))
								_positionSplit.x += 1;
							break;
					}
		...
		private bool[] TileCheck(int x, int y) {
		...
		private TileType ConfigureTile(int x, int y, bool[] tileFlags) {
		...
		private bool QuandrantCheck(int quandrant) {
			// Quandrants
			//   4 | 1
			//   -----
			//   3 | 2
		...
		private int CalculateDecorAmount(float decorPercent) {
			float numOfDecors = 0;
			float numOfEmpty = 0;
		...
		/// </summary>
		private void PlaceDecorSequentially() {
		...
		/// </summary>
		private void PlaceDecor(int num) {
		...
		private bool PutDecor(int y, int x) {
			if (grid[y, x].tile == TileType.EmptySpace) {
		...
		private void PlaceDecor2(int num) {
		...
		private void PutDecor2(int x, int y, TileType decor) {
			if (grid[x, y].tile == TileType.EmptySpace) {
		...
		private void EmptyTileCount(int x, int y, Direction dir) {
			// check to the top
		...
		private bool[] HasEmptySurroundingTile(int x, int y) {
		...
		/// <returns>number of decorations as an int</returns>
		private int CalculateTowerAmount(float towerPercent) {
			float numOfTowers = 0;
			float numOfEmpty = 0;
		...
		/// </summary>
		private void PlaceTowerSequentially() {
		...
		private void SingleTower() {
			...
			while (!towerPlaced) {
				...
				if (grid[randomPos.x, randomPos.y].tile <= TileType.Split4Ways) {
					...
					// down
					else if (randomPos.x < GRID_WIDTH - 1 &&
						grid[randomPos.x + 1, randomPos.y].tile == TileType.EmptySpace)
						grid[randomPos.x + 1, randomPos.y].tile = TileType.Tower;
					// left
					...
					// right
					else if (randomPos.y < GRID_HEIGHT - 1 &&
						grid[randomPos.x, randomPos.y + 1].tile == TileType.EmptySpace)
						grid[randomPos.x, randomPos.y + 1].tile = TileType.Tower;

					// top left
					else if (randomPos.x > 0 && randomPos.y > 0 &&
						grid[randomPos.x - 1, randomPos.y - 1].tile == TileType.EmptySpace)
						grid[randomPos.x - 1, randomPos.y - 1].tile = TileType.Tower;
					// top right
					else if (randomPos.x > 0 && randomPos.y < GRID_HEIGHT - 1 &&
						grid[randomPos.x - 1, randomPos.y + 1].tile == TileType.EmptySpace)
						grid[randomPos.x - 1, randomPos.y + 1].tile = TileType.Tower;
					// bottom left
					else if (randomPos.x < GRID_WIDTH - 1 && randomPos.y > 0 &&
						grid[randomPos.x + 1, randomPos.y - 1].tile == TileType.EmptySpace)
						grid[randomPos.x + 1, randomPos.y - 1].tile = TileType.Tower;
					// bottom right
					else if (randomPos.x < GRID_WIDTH - 1 && randomPos.y < GRID_HEIGHT - 1 &&
						grid[randomPos.x + 1, randomPos.y + 1].tile == TileType.EmptySpace)
						grid[randomPos.x + 1, randomPos.y + 1].tile = TileType.Tower;
